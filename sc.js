function runSimulation() {
         ...
         function fifo(pages, frames) {
           ...
           pages.forEach((page, index) => {
             ...
             if (!isHit) {
               if (queue.length < frames) { queue.push(page); }
               else { queue.shift(); queue.push(page); }
               faults++;
             } else { hits++; }
           ...
           for (let i = 0; i < pages.length; i++) {
             ...
             if (memory.length < frames) { memory.push(page); }
               ...
               memory.forEach((mPage, idx) => {
                 ...
                 if (nextUse === -1) { indexToReplace = idx; return; }
                 if (nextUse > farthestUse) { farthestUse = nextUse; indexToReplace = idx; }
               });
